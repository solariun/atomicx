...
class ThreadEventual : public atomic
{
public:
    ThreadEventual(atomic_time nNice, const char* pszName) : stack{}, atomic (stack), m_pszName(pszName)
    {
...
void ListAllThreads()
{
    ...
    std::cout << "Atomic ctx_size: " << sizeof (atomic) << ", Lock state -> Locked: " << (bool) glock.IsLocked() << ", Shared: " << glock.IsShared() << std::endl;
    std::cout << "-------------------------------------------------------" << std::endl;

    for (auto& th : *(atomic::GetCurrent()))
    {
        std::cout << (atomic::GetCurrent() == &th ? "*  " : "   ") << th.GetID() << "\t" << th.GetName() << "\t, Nice: " << th.GetNice() << "\t, Stack: " << th.GetStackSize() << ", UsedStack: " << th.GetUsedStackSize() << "\t, Status: " << th.GetStatus() << ", Lock: " << (int) glock.IsLocked() << "/" << (int) glock.IsShared()  << "  LockReference: " << th.GetTagLock() << std::endl;
    }
